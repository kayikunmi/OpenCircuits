...
beeper = TONE(13)  # Buzzer initialized once
...
CHANGE_THRESHOLD = 1000  # Threshold for detecting significant change
...
while True:
    ...
    if abs(pot_value - last_pot_value) > CHANGE_THRESHOLD and (time.time() - last_change_time) > DEBOUNCE_TIME:
        ...
        # Play a sound after the rotation is complete
        beeper.freq(840)  # A standard tone (A4)
        time.sleep(5)  # Sound duration
        beeper.freq(1)  # Reset the frequency to a very low value to effectively turn off the sound
